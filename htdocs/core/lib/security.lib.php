...
function restrictedArea($user, $features, $objectid=0, $dbtablename='', $feature2='', $dbt_keyfield='fk_soc', $dbt_select='rowid', $objcanvas=null)
{
    ...
    if (! empty($objectid) && $objectid > 0)
    {
        foreach ($features as $feature)
        {
            ...
            $checkdefault = 'all other not already defined'; // Test on entity and link to Companies. Not allowed if link is empty (Ex: invoice, orders...).
            ...
            else if (! in_array($feature,$nocheck))	// By default we check with link to Companies
            {
                // If external user: Check permission for external users
                if ($user->societe_id > 0)
                {
