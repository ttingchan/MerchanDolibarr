 ...
 *  \ingroup    core
 *	\brief      File of class to build HTML component for Companies management
 */


/**
 *	Class to build HTML component for Companies management
...
class FormCompany
{
	...
	/**
	 *    	Return list of labels (translated) of Companies type
	 *
	...
	function select_state($selected='',$country_codeid=0, $htmlname='state_id')
	{
		...
		if ($result)
		{
			...
			if (! empty($htmlname) && $user->admin) $out.= "";
		}
		else
		{
	 ...
	 *  @param  string		$selected       Pre-selected Companies
	 *  @param  string		$htmlname       Name of HTML form
	 * 	@param	array		$limitto		Disable answers that are not id in this array list
	 *  @param	int			$forceid		This is to force another object id than object->id
	 * 	@return int The selected Companies ID
	 * 	TODO obsolete ?
	...
	function selectCompaniesForNewContact($object, $var_id, $selected='', $htmlname='newcompany', $limitto='', $forceid=0)
	{
		...
		if ($resql)
		{
			if ($conf->use_javascript_ajax && ! empty($conf->global->COMPANY_USE_SEARCH_TO_SELECT))
			{
				...
				print "\n".'<!-- Input text for Companies with Ajax.Autocompleter (selectCompaniesForNewContact) -->'."\n";
